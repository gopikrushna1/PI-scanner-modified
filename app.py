...
# Known NSE/BSE index symbols in FO bhavcopy (IDF type)
INDEX_SYMBOLS = {
...
def process_bhavcopy(bhav_file, df_json, target_expiry_index=0, min_underlying_move_pct=0.0, instrument_filter='All'):
    ...
    try:
        ...
        # --- Process Futures ---
        # Filter futures by instrument type
        if instrument_filter == 'Index Only':
            futures = df_bhav[df_bhav['FinInstrmTp'] == 'IDF'].copy()
        elif instrument_filter == 'Stocks Only':
            futures = df_bhav[df_bhav['FinInstrmTp'] == 'STF'].copy()
        else:
            futures = df_bhav[df_bhav['FinInstrmTp'].isin(['STF', 'IDF'])].copy()

        # Secondary symbol-level filter
        if instrument_filter == 'Index Only':
            futures = futures[
                futures['TckrSymb'].isin(INDEX_SYMBOLS) |
                futures['TckrSymb'].str.contains('NIFTY|SENSEX|MIDCAP|SMALLCAP|BANKEX', case=False, na=False)
            ]
        elif instrument_filter == 'Stocks Only':
            futures = futures[
                ~(futures['TckrSymb'].isin(INDEX_SYMBOLS) |
        ...
        # Apply same instrument filter to options
        if instrument_filter == 'Index Only':
            options = options[
                options['TckrSymb'].isin(INDEX_SYMBOLS) |
                options['TckrSymb'].str.contains('NIFTY|SENSEX|MIDCAP|SMALLCAP|BANKEX', case=False, na=False)
            ]
        elif instrument_filter == 'Stocks Only':
            options = options[
                ~(options['TckrSymb'].isin(INDEX_SYMBOLS) |
